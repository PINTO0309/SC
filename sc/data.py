...
class Sample:
    """Lightweight container describing a single cropped sitting frame."""
...
POSITIVE_LABELS = {"open", "sitting"}
NEGATIVE_LABELS = {"closed", "not_sitting"}
...
def _to_label(row: pd.Series) -> int:
    if "class_id" in row and pd.notna(row["class_id"]):
        ...
        if label_id not in (0, 1):
            raise ValueError(f"Unsupported class_id value: {label_id}")
        return label_id
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", "sitting" if label == 1 else "not_sitting"))
...
def split_samples(
    ...
    for split, subset in assignments.items():
        ...
        logger.info(
            "Split '%s': %d samples from %d sources (%.1f%% labelled 'sitting').",
            split,
            total,
...
class SCDataset(Dataset):
    """Torch dataset that reads sittings crops from disk or embedded bytes."""
